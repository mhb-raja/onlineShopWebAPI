...
{
    public class Slider2Controller : SiteBaseController
    {
        #region constructor

        private readonly ISliderService2 sliderService;
        public Slider2Controller(ISliderService2 sliderService)
        {
        ...
        public async Task<IActionResult> AddSlider([FromBody] SliderDTO slider)
        {
            ...
            {                
                return JsonResponseStatus.Success(await sliderService.AddSlider(slider));
            }
        ...
        public async Task<IActionResult> EditSlider([FromBody] SliderDTO slider)
        {
            if (ModelState.IsValid)
            {                
                return JsonResponseStatus.Success(await sliderService.EditSlider(slider));
            }
            return JsonResponseStatus.Error();
        }
        #endregion
